...
create index permissions_space_id_idx on public.permissions (space_id);
...
create index pending_invites_space_id_idx on public.pending_invites (space_id);
...
create index pages_space_id_idx on public.pages (space_id);
...
create index pages_publishable_published_idx on public.pages (visibility, is_published)
...
create table public.screenshot_blocks (
  id uuid primary key default gen_random_uuid(),
...
create index screenshot_blocks_page_id_idx on public.screenshot_blocks (page_id);
...
create table public.versions (
  ...
  created_by_user_id uuid not null references public.profiles (id),
  created_at timestamptz not null default now(),
  is_restore_of uuid references public.versions (id)
);

create index versions_page_id_idx on public.versions (page_id, created_at desc);
...
create index comments_page_id_idx on public.comments (page_id, block_id);
...
create index feedback_page_id_idx on public.feedback (page_id);
...
create trigger screenshot_blocks_set_updated_at
  before update on public.screenshot_blocks
  for each row execute function public.set_updated_at();
