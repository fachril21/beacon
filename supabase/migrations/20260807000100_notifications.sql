...
create table beacon.notifications (
  id uuid primary key default gen_random_uuid(),
  recipient_user_id uuid not null references beacon.profiles (id) on delete cascade,
  actor_user_id uuid not null references beacon.profiles (id),
  page_id uuid not null references beacon.pages (id) on delete cascade,
  comment_id uuid not null references beacon.comments (id) on delete cascade,
  is_read boolean not null default false,
  created_at timestamptz not null default now()
);

create index notifications_recipient_idx on beacon.notifications (recipient_user_id, is_read, created_at desc);

alter table beacon.notifications enable row level security;

-- A User only ever sees their own notifications.
create policy notifications_select_own
  on beacon.notifications for select
  to authenticated
...
create policy notifications_insert_by_commenter
  on beacon.notifications for insert
  to authenticated
  with check (
    actor_user_id = auth.uid()
    and beacon.user_space_role(beacon.page_space_id(page_id)) is not null
  );
...
create policy notifications_update_own_mark_read
  on beacon.notifications for update
  to authenticated
  using (recipient_user_id = auth.uid())
  with check (recipient_user_id = auth.uid());

grant select, insert, update on beacon.notifications to authenticated, service_role;
